...
# codex round 3 [NIT #2]: model_types served by rapid_mlx.models.* shims.
...
#
# Initialized with the archs whose vendor modules ship inside rapid_mlx and
# never need dynamic registration (``deepseek_v4`` is pure Python inside
# ``rapid_mlx.models`` — if that import fails, the wheel is broken and the
...
def _register_vendored_archs() -> None:
    ...
    if "mlx_lm.models.muse_glimmer" not in sys.modules:
        ...
        # ``rapid_mlx/models/muse_glimmer.py`` for the why + sync policy).
    ...
    if "mlx_lm.models.g9v3" not in sys.modules:
        ...
        # ``rapid_mlx/models/g9v3.py`` for the why + sync policy). Defers
    ...
    if "mlx_lm.models.bailing_hybrid" not in sys.modules:
        ...
        # KDA+MLA hybrid backbone (see ``rapid_mlx/models/bailing_hybrid.py``
    ...
    if "mlx_lm.models.k2_horizon" not in sys.modules:
        ...
        if _k2_native_spec is None:
            try:
                ...
                _k2_horizon = _importlib.import_module("rapid_mlx.models.k2_horizon")
...
def _post_load_ubc_evict(model_name: str) -> None:
    ...
    No-op on non-Darwin platforms (see :mod:`rapid_mlx.runtime.ubc_evict`).
...
def load_model_with_fallback(
    ...
    *,
    ...
    Args:
        ...
        lazy: ``--disk-stream`` path only. When True, skip every
            *branch-selection* fallback / tokenizer-quirk path below
            ...
            loaded model. ``rapid_mlx.disk_stream_patch.install`` is
    ...
    if lazy:
        ...
        # --disk-stream architectures (rapid_mlx/registry.py) — without
        ...
        # and `qwen3_next` are, per rapid_mlx/registry.py), so none of these
