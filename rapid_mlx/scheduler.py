...
class SchedulerConfig:
    ...
    # ``rapid_mlx.cli`` resolve dtype → (quantization, bits) via
    # :func:`rapid_mlx.kv_cache_dtype.dtype_to_quantization_bits` so the
    # two stay coherent.
    ...
    # see rapid_mlx/pflash.py for the design notes and the prefix-cache
    ...
    # ``rapid_mlx/spec_decode/mtp/detect.py::detect_mtp_eligibility``
    ...
    # Consumed at the route layer (``rapid_mlx/response_cache.py`` singleton,
...
def _install_mtp_vendored(
    ...
    Gate (all required):
      * ``model`` exposes the ``mtp_generate_step`` protocol:
        ...
        :func:`~rapid_mlx.spec_decode.mtp.dispatch.dispatch_mtp_inject`).
...
class Scheduler:
    ...
    def __init__(
        ...
        Args:
            ...
            model_config: Optional ``ModelConfig`` from
                ``rapid_mlx.model_auto_config``. Used as a capability gate for
    ...
    def _get_request_sampler(self, sampling_params: SamplingParams) -> Any:
        ...
        # without top_k). See ``rapid_mlx/_sampler_fast_path.py`` for the
            ...
            for layer_state in extracted_states:
                ...
                if cache_cls is not None and hasattr(cache_cls, "from_state"):
                    ...
                    if cache_cls is _QuantizedBatchKVCache:
                        ...
                        from rapid_mlx.memory_cache import _vendored_cache_registry
