...
# ``rapid_mlx.service.helpers`` (route helpers + their tests) continue
# to work unchanged.
...
#: :mod:`rapid_mlx.api.constants` — kept at module scope so existing
#: callers (route helpers + tests) continue to import it from
#: ``rapid_mlx.service.helpers``. The Anthropic adapter consumes the
...
def _is_structured_output_requested(response_format) -> bool:
    ...
    ``rapid_mlx/routes/chat.py`` to decide whether to suppress the
...
def _resolve_enable_thinking(request) -> bool | None:
    ...
    ``rapid_mlx/speculative/dflash/server.py`` for that path.
    """
    cfg = get_config()
    if cfg.no_thinking:
        return False
...
def _effective_enable_thinking(
    ...
    ``rapid_mlx.utils.chat_template.apply_chat_template`` uses when
...
# context window. The body-size middleware ``rapid_mlx/middleware/body_size.py``
