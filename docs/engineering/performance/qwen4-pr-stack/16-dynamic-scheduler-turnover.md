...
Depends on: PR 15 / publication fixed-cohort live-delivery split.
...
Once fixed-cohort delivery owns the live response path, Qwen3.5 can reuse the
wrapper's closed-boundary attach/detach transaction to replace finished lanes
without restarting survivor requests.
